                ...
                <div className="space-y-6 flex flex-col flex-1">
                    <div className="bg-gray-900 rounded-lg p-6">
                        <div className="flex flex-row font-play mb-4 mt-4 border border-[#777] rounded-lg text-center">
                            <div className={`basis-1/2 py-4 rounded-l-lg cursor-pointer  ${status === 'deposit' && 'bg-cyan-500 text-gray-800 font-bold'}`} onClick={() => setStatus('deposit')}>{t("ickbForm.deposit")}</div>
                            <div className={`basis-1/2 py-4 rounded-r-lg cursor-pointer  ${status === 'withdraw' && 'bg-cyan-500 text-gray-800 font-bold'}`} onClick={() => setStatus('withdraw')}>{t("ickbForm.withdraw")}</div>
                        </div>
                    ...
                    {(ickbData && ickbData.myMaturity.length) ?
                        <div className="bg-gray-900 rounded-lg p-6 flex flex-col  mt-6">
                            <h3 className="text-xl font-play font-bold mb-4">{t("ickbForm.pendingNervosDAOWithdraw")}</h3>
                            <div>
